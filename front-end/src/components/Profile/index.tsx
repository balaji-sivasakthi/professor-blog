...
const Profile = () => {
    const [activeComponent, setActiveComponent] = useState<T | null>(<Positions />)
    const data = [
        {
                ...
                <ul className="flex flex-wrap w-full justify-evenly -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700" >
                    {
                        data.map((item: any) => {
                            return (
                                <>
                                    <li className="me-2 hover:bg-gray-300 rounded-lg" onClick={() => setActiveComponent(item.component)}>
